...
ob_start();
 ...
 if(isset($_REQUEST['criteria']) AND isset($_REQUEST['user']) ){
    ...
    $no =0;
    $stms_doctor = $conn->prepare(" SELECT * FROM park_in WHERE ".$criteria." AND status = 0 ");                     
    try {
    ...
    $this->Image('../img/logo.jpg',10,10,30);
        ...
        $this->Cell(60);
        //Title
           $this->Cell(157,7,'PARKING LEFT REPORT',0,0,'C');
    ...
    $pdf->SetTitle(" PARKING LEFT REPORT ");
    	...
    	$pdf->SetTextColor(0,0,0);	
    	$pdf->Cell(165, 1," Parking Left Lists ", 0, 1, 'R');
    	...
    	$pdf->Cell(10, 7, '');
    	$pdf->Cell(10, 7, 'No', 1);
    	$pdf->Cell(30, 7, 'License Plate', 1);
    	$pdf->Cell(30, 7, 'Phone', 1);
    	$pdf->Cell(40, 7, 'Time In', 1);
    	$pdf->Cell(30, 7, 'Time Used', 1);
    	$pdf->Cell(30, 7, 'Pay for', 1);
    	$pdf->Cell(30, 7, 'Amount', 1);
    	$pdf->Cell(60, 7, 'Served By', 1);
        ...
        while($rows_2 = $stms_doctor->fetch(PDO::FETCH_ASSOC)) {   
            ...
            $diff_time = strtotime($rows_2['entre_time']) - strtotime($curr_time);
            $timeUsed = gmdate('H:i:s', abs($diff_time));
            
            $tempTotalTime = differenceInHours($rows_2['entre_time'], $curr_time);
        	...
        	$pdf->Cell(10, 7, '');
        	$pdf->Cell(10, 7, $no, 1);
        	$pdf->Cell(30, 7, $rows_2['card_code'], 1);
        	$pdf->Cell(30, 7, $rows_2['phone'], 1);
        	$pdf->Cell(40, 7, $rows_2['entre_time'], 1);
        	$pdf->Cell(30, 7, $timeUsed, 1);
        	$pdf->Cell(30, 7, $tempTotalTime."(Hours)", 1);
        	$pdf->Cell(30, 7, $pay['price'], 1);
        	$pdf->Cell(60, 7, $opdata['sg_name'], 1);
        	...
        	$pdf->SetTextColor(0, 0, 0);
        	$pdf->SetFont('Times', 'B', 12);
            $pdf->Cell(40,3,'Total Amount : '.$total_amount.' Frw',0,1,'L');
    ...
    $n1 = $tech_name." PARKING LEFT REPORT ";
    $pdf->Output($n1 . '.pdf', 'I');
    $pdf->Output();
